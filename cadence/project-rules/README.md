...
> 以下为合并前本目录 README 已有内容，按原顺序保留；与模板完全重复的部分已去重。

- 原说明：本目录用于存放项目的个性化规则和模板，由 `cadence-init:project-rules-examples` skill 填充详细内容。
- 原「目录说明」章节两条要点（`cadence/project-rules/` 为用户自定义规则和模板；`.claude/rules/` 为框架内置规则、禁止直接修改）与上方「规则目录划分」表内容重复，已去重。
